...
class LLM:
    ...
    """
    @overload
    def __new__(cls, model: str=None, type: Literal["local"]="local", **kwargs) -> 'LocalLLM': ...
    @overload
    def __new__(cls, model: str=None, type: Literal["remote"]="remote", platform: PlatformType = ..., **kwargs) -> 'RemoteLLM': ...
    def __new__(cls, model: str=None, type: ModelType = "local", platform: PlatformType = "VLLM", **kwargs):
        if type == "local":
            from .local_llm import LLM as LocalLLM
